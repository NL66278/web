openerp.web_attachment_size_limit = function(openerp) {
        ...
        on_attachment_changed: function(e) {
            ...
            var loaded = self.fetch(
                ...
                [['id','=',this.session.uid]]
                    ...
                    var att_size=self.fetch(
                        'res.company',
                        ['attachment_size','attachment_num','user_blocked'],
                        [['id','=',users[0].company_id[0]]]
                    ).then(function(company){
                        attachment_size=company[0].attachment_size;
                        user_dict={}
                        for(var i=0; i<company[0].user_blocked.length;i++) {
                            user_dict[company[0].user_blocked[i]]="";
                        }
                        ...
                        if(company[0].attachment_num<total_att) {
                            ...
                            openerp.webclient.notification.warn(
                                _t("File upload"),
                                _.str.sprintf(msg + (company[0].attachment_num) + " files allowed","")
                            );
                            return false;
                        }
                        if (attachment_size == 0) {
                            ...
                            openerp.webclient.notification.warn(
                                _t("File upload"),
                                _.str.sprintf(msg, attachment_size +" MB")
                            );
                            return false;
                        }
                        if ($e.val() !== '') {
